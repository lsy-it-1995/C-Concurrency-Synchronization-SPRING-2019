...
void* display_task(void* data) {
    ...
    start_time = (long)lap.tv_sec * 1000 + (long)lap.tv_usec / 1000;
    // setup the display
    setup_ncurses(*(int*)data);
    gettimeofday(&lap, NULL);
    long laptime = (long)lap.tv_sec * 1000 + (long)lap.tv_usec / 1000;
    mvwprintw(msgwin, 4, 2, "Elapsed time (s): %0.2f", ((double)(laptime - start_time))/1000);
    wrefresh(msgwin);
    free(data);
    int i;
